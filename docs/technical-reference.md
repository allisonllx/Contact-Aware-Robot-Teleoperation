...
| `--hold-teleop` | Interactive runs | Enable continuous hold-to-move through `pynput`. Off by default for cleaner trial timing. |
| `--actuator-boost` | Interactive runs | Set arm actuator gain scaling. Lower values reduce lurching but feel softer. Default is `4.0`. |
